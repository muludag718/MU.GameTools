...
namespace MU.GameTools.Squish
{
    public class DDSFile
    {
        ...
        public Image Image(bool red, bool green, bool blue, bool alpha)
        {
            ...
            for (int i = 0; i < height; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    ...
                    int red2 = red ? pixelData[num] : 0;
                    int green2 = green ? pixelData[num + 1] : 0;
                    int blue2 = blue ? pixelData[num + 2] : 0;
                    int alpha2 = alpha ? pixelData[num + 3] : 0;

                    bitmap.SetPixel(j, i, alpha
                        ? Color.FromArgb(alpha2, red2, green2, blue2)
        ...
        public void Deserialize(Stream input)
        {
            ...
            // Buradaki format kontrolleri değişmedi
            if (Header.PixelFormat.Flags == PixelFormatFlags.RGBA && Header.PixelFormat.RGBBitCount == 32 &&
                Header.PixelFormat.RedBitMask == 16711680 && Header.PixelFormat.GreenBitMask == 65280 &&
                Header.PixelFormat.BlueBitMask == 255 && Header.PixelFormat.AlphaBitMask == 4278190080u)
            {
                fileFormat = FileFormat.A8R8G8B8;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGB && Header.PixelFormat.RGBBitCount == 32 &&
                     Header.PixelFormat.RedBitMask == 16711680 && Header.PixelFormat.GreenBitMask == 65280 &&
                     Header.PixelFormat.BlueBitMask == 255 && Header.PixelFormat.AlphaBitMask == 0)
            {
                fileFormat = FileFormat.X8R8G8B8;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGBA && Header.PixelFormat.RGBBitCount == 32 &&
                     Header.PixelFormat.RedBitMask == 255 && Header.PixelFormat.GreenBitMask == 65280 &&
                     Header.PixelFormat.BlueBitMask == 16711680 && Header.PixelFormat.AlphaBitMask == 4278190080u)
            {
                fileFormat = FileFormat.A8B8G8R8;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGB && Header.PixelFormat.RGBBitCount == 32 &&
                     Header.PixelFormat.RedBitMask == 255 && Header.PixelFormat.GreenBitMask == 65280 &&
                     Header.PixelFormat.BlueBitMask == 16711680 && Header.PixelFormat.AlphaBitMask == 0)
            {
                fileFormat = FileFormat.X8B8G8R8;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGBA && Header.PixelFormat.RGBBitCount == 16 &&
                     Header.PixelFormat.RedBitMask == 31744 && Header.PixelFormat.GreenBitMask == 992 &&
                     Header.PixelFormat.BlueBitMask == 31 && Header.PixelFormat.AlphaBitMask == 32768)
            {
                fileFormat = FileFormat.A1R5G5B5;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGBA && Header.PixelFormat.RGBBitCount == 16 &&
                     Header.PixelFormat.RedBitMask == 3840 && Header.PixelFormat.GreenBitMask == 240 &&
                     Header.PixelFormat.BlueBitMask == 15 && Header.PixelFormat.AlphaBitMask == 61440)
            {
                fileFormat = FileFormat.A4R4G4B4;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGB && Header.PixelFormat.RGBBitCount == 24 &&
                     Header.PixelFormat.RedBitMask == 16711680 && Header.PixelFormat.GreenBitMask == 65280 &&
                     Header.PixelFormat.BlueBitMask == 255 && Header.PixelFormat.AlphaBitMask == 0)
            {
                fileFormat = FileFormat.R8G8B8;
            }
            else if (Header.PixelFormat.Flags == PixelFormatFlags.RGB && Header.PixelFormat.RGBBitCount == 16 &&
                     Header.PixelFormat.RedBitMask == 63488 && Header.PixelFormat.GreenBitMask == 2016 &&
                     Header.PixelFormat.BlueBitMask == 31 && Header.PixelFormat.AlphaBitMask == 0)
            {
            ...
            int num5 = ((Header.Flags & HeaderFlags.Pitch) != 0) ? (int)Header.PitchOrLinearSize :
                       (((Header.Flags & HeaderFlags.LinerSize) == 0) ? (Header.Width * num4) : ((int)Header.PitchOrLinearSize / Header.Height));
